	...
	<view>
		...
		<view class="yt-list">
			<view class="yt-list-cell b-b">
				...
				<text class="cell-tip">￥{{getOrderPrice.order_price}}</text>
			</view>
			<view class="yt-list-cell b-b">
				<text class="cell-tit clamp">优惠金额</text>
				<text class="cell-tip red">￥{{getOrderPrice.discount_price}}</text>
			</view>
			<view class="yt-list-cell b-b">
		...
		<view class="footer">
			<view class="price-content">
				...
				<text class="price">{{getOrderPrice.products_price}}</text>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			async loadData(msg){
				// 创建订单
				await this.$store.dispatch("cart/CreateOrder",msg)
				await this.$store.dispatch('user/getAddress');
				//获取用户默认地址信息 没有默认则获取第一个
				this.addressData = this.addressInfo.filter(item => item.is_default == 1)[0] || this.addressInfo[0]
				//获取订单总金额
				await this.$store.dispatch('order/OrderPrice',Object.assign({},this.params,{
			...
			// 提交订单
			async submit(){
				const {data} = await this.$store.dispatch('order/SubmitOrder',Object.assign({},this.params,{
					address_id: this.addressData.id,
				}));
				if (data.code == 1) {
					let price = this.getOrderPrice.order_price
					uni.redirectTo({
